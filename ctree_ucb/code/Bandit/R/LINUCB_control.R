LINUCB_control <- function(dt,visitorReward, K ){
  LINUCB_control_K(visitorReward = visitorReward, K= K)
  LINUCB_control_data(dt=dt,visitorReward = visitorReward, K=K)
}


LINUCB_control_data <- function(dt,visitorReward,K){
  ...
  for(i in 1:ncol(visitorReward)){
    try(if(length(intersect(typeof(visitorReward[,i]),list_reward_possible))==0 ) stop(paste("The",i,"colomn of reward data is not a double of integer data",sep=" ")))
    }


  #Type controle contextual data
  list_context_possible <- c("double","integer")
  for(i in 1:ncol(dt)){
    try(if(length(intersect(typeof(dt[,i]),list_context_possible))==0 ) stop(paste("The",i,"colomn of context data is not a double of integer data",sep=" ")))
  }


  #Reward data type identicals control
  type_of_reward <- sapply(visitorReward, typeof)
  try( if(length(unique(type_of_reward)) > 1) stop("reward data type are not identicals "))
...
LINUCB_control_K <- function( visitorReward, K){
